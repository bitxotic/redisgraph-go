...
// Return a slice containing the results of the query in as generic a form as possible.
func (qr *QueryResult) QueryRecords() ([]interface{}, error) {
	var results []interface{}
	if qr.Empty() {
		return results, fmt.Errorf("no records generated by query")
	} 
	if len(qr.results) > 0 {
		for _, record := range qr.results {
